...
resource "aws_route53_record" "dns_records_for_https_certificate_validation" {
  for_each = {
...
resource "aws_acm_certificate_validation" "certificate_validation_waiter" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.dns_records_for_https_certificate_validation : record.fqdn]
}
